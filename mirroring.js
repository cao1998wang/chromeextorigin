...
pb.addEventListener('signed_in', function(e) {
    pb.addEventListener('stream_message', function(e) {
        ...
        if (push.type == 'mirror') {
            mirrorReceived(utils.decryptIfNeeded(push));
        } else if (push.type == 'dismissal') {
            dismissalReceived(utils.decryptIfNeeded(push));
        }
    });
});
...
var muteButton = function(mirror) {
    var mute = function() {
        pb.post(pb.api + '/v2/ephemerals', {
            ...
            'push': {
                'type': 'mute',
                'encrypted': pb.e2e.enabled,
                'source_user_iden': mirror.source_user_iden,
                'package_name': pb.e2e.optEncrypt(mirror.package_name)
            }
        }, function(response) {
            if (response) {
...
var dismissRemote = function(mirror, triggerKey) {
    var dismissal = {
        ...
        'source_user_iden': mirror.source_user_iden,
        'package_name': pb.e2e.optEncrypt(mirror.package_name),
        'notification_id': pb.e2e.optEncrypt(mirror.notification_id),
        'notification_tag': pb.e2e.optEncrypt(mirror.notification_tag)
    };

    if (mirror.conversation_iden) {
        dismissal.conversation_iden = pb.e2e.optEncrypt(mirror.conversation_iden);
    }

    if (triggerKey) {
        dismissal.trigger_action = pb.e2e.optEncrypt(triggerKey);
    }
...
pb.sendReply = function(mirror, message) {
    pb.post(pb.api + '/v2/ephemerals', {
        ...
        'push': {
            ...
            'package_name': pb.e2e.optEncrypt(mirror.package_name),
            'conversation_iden': pb.e2e.optEncrypt(mirror.conversation_iden),
            'message': pb.e2e.optEncrypt(message)
        }
    }, function(response) {
        if (response) {
