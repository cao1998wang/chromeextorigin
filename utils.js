...
var encryptedKeys = ['title', 'body', 'icon', 'application_name', 'package_name', 'notification_id', 'notification_tag',
                     'conversation_iden', 'thread', 'threads'];
utils.decryptIfNeeded = function(obj) {
    if (!obj) {
        return obj;
    }
    ...
    if (!pb.e2e.enabled) {
        var options = {
            'type': 'basic',
            'key': 'e2e',
            'title': text.get('encryption_password_needed_title'),
            'message': text.get('encryption_password_needed_body'),
    ...
    try {
        Object.keys(obj).forEach(function(key) {
            if (encryptedKeys.indexOf(key) != -1) {
                obj[key] = pb.e2e.decrypt(obj[key]);
            }
        });
        ...
        if (actions) {
            actions.forEach(function(action) {
                action.label = pb.e2e.decrypt(action.label);
                action.trigger_key = pb.e2e.decrypt(action.trigger_key);
            });
        }

        var threads = obj.threads;
        if (threads) {
            obj.threads = JSON.parse(obj.threads);
        }

        var thread = obj.thread;
        if (thread) {
            obj.thread = JSON.parse(obj.thread);
        }
    } catch (e) {
        var options = {
            'type': 'basic',
            'key': 'e2e',
            'title': text.get('decryption_failed_title'),
            'message': text.get('decryption_failed_body'),
            'contextMessage': 'Pushbullet',
            'iconUrl': 'lock@2x.png',
            'onclick': function() {
                if (window.chrome) {
